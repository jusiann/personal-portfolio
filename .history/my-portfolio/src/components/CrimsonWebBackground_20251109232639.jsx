...
export const CrimsonWebBackground = () => {
  ...
  const generateConnections = useCallback((nodeList) => {
    ...
    const maxDistance = 22; // Biraz daha uzun bağlantılar
    const maxConnectionsPerNode = 5; // Her düğüm max 5 bağlantı
    const nodeConnections = new Array(nodeList.length).fill(0);

    for (let i = 0; i < nodeList.length; i++) {
      for (let j = i + 1; j < nodeList.length; j++) {
        if (nodeConnections[i] >= maxConnectionsPerNode || 
            nodeConnections[j] >= maxConnectionsPerNode) {
          continue;
        }
