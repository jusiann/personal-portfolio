...
function SkillsSection() {
    ...
    const allSkills = {
        frontend: skillsData.frontend.map(skill => ({...skill, icon: getSkillIcon(skill.icon)})),
        backend: skillsData.backend.map(skill => ({...skill, icon: getSkillIcon(skill.icon)})),
        tools: skillsData.tools.map(skill => ({...skill, icon: getSkillIcon(skill.icon)}))
    };
                    ...
                    <svg className="w-14 h-14" viewBox="0 0 44 44">
                        {[...Array(totalSegments)].map((_,index) => (
                            <path
                                ...
                                strokeWidth="2"
                                strokeLinecap="round"
                                className={index < filledSegments ? "text-primary" : "text-foreground/15"}
                            />
                        ))}
                    </svg>
