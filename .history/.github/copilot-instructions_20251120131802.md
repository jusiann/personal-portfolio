...
**Remove AI Code Slop**: When reviewing changes, eliminate verbose AI-generated patterns:
- Excessive comments that are obvious from code context (e.g., `{/* Submit button */}` above `<button type="submit">`)
- Overly descriptive comments in Turkish or English that don't add value
- Unnecessary try/catch blocks in simple, trusted code paths
- Defensive type checks where types are already validated
- `any` type casts to bypass TypeScript errors

**Clean Code Principles**:
- Keep comments minimal and meaningful - explain "why", not "what"
- Trust validated inputs; don't add redundant safety checks
- Let code be self-documenting through clear naming
- Match existing file style and comment density
